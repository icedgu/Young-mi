...
public final class EyesActivity extends AppCompatActivity {
    ...
    private FaceDetector createFaceDetector(Context context) {
        ...
        tracker.setCheckStateListener(new FaceTracker.CheckStateListener() {
            @Override
            public void checkState() {
                Log.d("test@", "in");
                ToneGenerator toneGenerator = new ToneGenerator(AudioManager.STREAM_MUSIC, ToneGenerator.MAX_VOLUME);
                toneGenerator.startTone(ToneGenerator.TONE_DTMF_S, 500);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(EyesActivity.this, "졸음감지", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
